...
TEST_DATA="../../test-data/homereef-transflux"
...
function dump_logs() {
    local LOG_DIR="../../test-data/homereef-transflux/logs"
    mkdir -p "$LOG_DIR"
    local TIMESTAMP
    TIMESTAMP=$(date +%Y%m%d_%H%M%S)
    echo "Dumping container logs to $LOG_DIR/e2e_${TIMESTAMP}.log ..."
    podman logs homereef-e2e_homereef_1 > "$LOG_DIR/e2e_${TIMESTAMP}.log" 2>&1 || true
}
...
function run_e2e() {
    ...
    if podman ps --format '{{.Names}}' | grep -q "homereef-e2e_"; then
