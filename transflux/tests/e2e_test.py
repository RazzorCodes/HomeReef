...
def check_output_exists():
    ...
    dest_dir = os.path.join(os.path.dirname(__file__), "..", "..", "test-data", "homereef-transflux", "media")
...
def check_source_consumed():
    """Verify the source file was consumed (deleted) by the transflux."""
    source_dir = os.path.join(os.path.dirname(__file__), "..", "..", "test-data", "homereef-transflux", "media")
    ...
    if "test_video_h264.mkv" not in source_files:
        print("Source file was consumed (deleted) by the transflux — OK")
        return True
    else:
